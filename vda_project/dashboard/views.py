...
def document_edit_view(request, vehicle_pk, pk):
    document = get_object_or_404(Document, pk=pk, vehicle__pk=vehicle_pk, vehicle__owner=request.user)
    if request.method == 'POST':
...
def maintenance_edit_view(request, vehicle_pk, pk):
    record = get_object_or_404(Maintenance, pk=pk, vehicle__pk=vehicle_pk, vehicle__owner=request.user)
    if request.method == 'POST':
...
def maintenance_alert_edit_view(request, vehicle_pk, pk):
    alert = get_object_or_404(MaintenanceAlert, pk=pk, vehicle__pk=vehicle_pk, vehicle__owner=request.user)
    if request.method == 'POST':
