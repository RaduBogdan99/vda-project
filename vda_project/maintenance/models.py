...
class Maintenance(models.Model):
    ...
    odometer = models.PositiveIntegerField(
        verbose_name="Kilometraj (km)", help_text="Kilometrajul la momentul operațiunii"
    )
...
class MaintenanceAlert(models.Model):
    ...
    vehicle = models.ForeignKey(
        Vehicle, on_delete=models.CASCADE, related_name="maintenance_alerts", verbose_name="Vehicul"
    )
    ...
    km_interval = models.PositiveIntegerField(
        ...
        null=True,
        blank=True,  # Poate fi o alertă bazată doar pe timp
        help_text="Ex: 10000",
    )
    months_interval = models.PositiveIntegerField(
        verbose_name="Interval Timp (luni)",
        null=True,
        blank=True,  # Poate fi o alertă bazată doar pe km
        help_text="Ex: 12",
    )
